...
export const addToFavorites = (productId) => async (dispatch) => {
  dispatch(addFavorite({ product_id: productId })); // Optimistically add
  try {
    const response = await fetch(`/api/favorites/${productId}`, { method: "POST" });
    if (!response.ok) throw new Error("Failed to add to favorites");
    const newFavorite = await response.json();
    dispatch(addFavorite(newFavorite)); // Update if needed
  } catch (error) {
    dispatch(removeFavorite(productId)); // Revert if it fails
    dispatch(favoriteError(error.message));
  }
};

export const removeFromFavorites = (productId) => async (dispatch) => {
  dispatch(removeFavorite(productId)); // Optimistically remove
  try {
    const response = await fetch(`/api/favorites/${productId}`, { method: "DELETE" });
    if (!response.ok) throw new Error("Failed to remove from favorites");
  } catch (error) {
    // Re-add the favorite in case of failure
    dispatch(fetchFavorites()); // Re-fetch to ensure consistency
    dispatch(favoriteError(error.message));
  }
};
